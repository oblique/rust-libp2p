...
pub struct Stream {
    ...
    read_leftovers: Option<SendWrapper<Uint8Array>>,
...
impl Drop for Stream {
    fn drop(&mut self) {
        // On drop we abort any writes and cancel any ongoing reads
        detach_promise(self.writer.abort());
...
impl AsyncRead for Stream {
    fn poll_read(
        ...
        if leftovers.byte_length() > 0 {
            this.read_leftovers = Some(SendWrapper::new(leftovers));
        }
